# Adolfo 8/9
# This is the python script for *only* training the Kassandra model.
# I'm just copying over from the kassandra_model_training_example.py script, but removing the prediction part.
# The prediction will be in a separate file.
...
mixer = Mixer(cell_types=cell_types,
              cells_expr=normal_cells_rows_expr, cells_annot=normal_cells_rows_anno,
              tumor_expr=cancer_cells_rows_expr, tumor_annot=cancer_cells_rows_anno,
              num_av = 3, num_points = 3000) # Changed from 300,000 to 3,000 to reduce training time for testing
...
with open('models/tumor_model.pkl', 'wb') as f:
